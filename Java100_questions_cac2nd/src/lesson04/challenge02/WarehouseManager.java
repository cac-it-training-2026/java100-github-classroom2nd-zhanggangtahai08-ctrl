...
public class WarehouseManager {
	...
	public static void main(String[] args) throws IOException {
		...
		int choice = Integer.parseInt(numberString);

		System.out.print("\n要素数を選んでください（1...1個、2...2個、3...3個）＞");

		//ここに入力処理を記述する。
		String numberString2 = br.readLine();
		int count = Integer.parseInt(numberString2);

		boolean errFlag = false;

		//ここに入力値の範囲チェック処理を記述する。
		if ((count > 3 || count < 1) || choice > 3 || choice < 1) {
			errFlag = true;
		}
		...
		if (!errFlag) {
			...
			if (choice == 1) {
				System.out.println(charArray[choice - 1]);
			} else if (choice == 2) {
				System.out.println(strArray[choice - 1]);
			} else if (choice == 3) {
				System.out.println(intArray[choice - 1]);
			}
