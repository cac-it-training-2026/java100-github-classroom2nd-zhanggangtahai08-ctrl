...
public class WarehouseManager {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < jankenArray1.length; i++) {
			do {
				...
				if (Number != 1 && Number != 2 && Number != 3) {
					...
					loopFlag = true;
				} else if ((stoneCount1 == 2) && Number == 1) {
					System.out.println("\nYさん：");
					System.out.println("グーはもう使えません。");
					loopFlag = true;
				} else if ((scissorsCount1 == 2) && Number == 2) {
					System.out.println("\nYさん：");
					System.out.println("チョキはもう使えません。");
					loopFlag = true;
				} else if ((paperCount1 == 2) && Number == 3) {
					System.out.println("\nYさん：");
		...
		for (int i = 0; i < jankenArray2.length; i++) {
			do {
				loopFlag = false;
				intputNum = (int) (Math.random() * 10 % 3 + 1);
				if ((stoneCount2 == 2) && intputNum == 1) {
					loopFlag = true;
				} else if ((scissorsCount2 == 2) && intputNum == 2) {
					loopFlag = true;
				} else if ((paperCount2 == 2) && intputNum == 3) {
					loopFlag = true;
				}

			} while (loopFlag);
			jankenArray2[i] = intputNum;
			if (intputNum == 1) {
				stoneCount2++;
			} else if (intputNum == 2) {
				scissorsCount2++;
		...
		for (int count = 0; count < 5; count++) {
			...
			do {
				...
				String choice = br.readLine();
				openNum1 = Integer.parseInt(choice) - 1;
